...
- **4-1. 실시간 데이터 저장:**
    - 10초마다 생성된 데이터를 메모리에 보관하고, N분마다 DB에 일괄 저장(`batch insert`)하는 로직을 구현합니다.
- **4-2. 과거 데이터 생성 (최초 실행):**
    - 애플리케이션이 처음 시작될 때, 지난 N년 치의 과거 사용량 데이터를 생성하여 DB에 저장하는 로직을 구현합니다. 이 작업은 `ApplicationRunner`를 활용하여 구현할 수 있습니다.
- **4-3. 누락 데이터 보완 (재실행 시):**
    - 애플리케이션 재시작 시, 지난 N년간의 데이터 중 누락된 부분이 있는지 확인하고, 규칙에 맞게 데이터를 채워 넣는 로직을 구현합니다.
...
- **5-2. 비교군 빌딩 과거 데이터 생성:**
    - 생성된 100개 빌딩 각각에 대해, 운영 시간, 피크 시간, 계절적 요인을 고려하여 1시간 단위의 지난 3년 치 사용량(전력, 수도, 가스) 데이터를 생성하고 DB에 저장합니다. 
    - 이 로직은 빌딩 전체 사용량(각 전력, 가스, 수도)만 계산합니다.
    - 정규 분포를 따르도록 데이터를 생성합니다. 
